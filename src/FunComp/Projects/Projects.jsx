  ...
  return (
    ...
    <div className="my-10 flex flex-col items-center justify-center gap-10 w-full">
      <div className="flex flex-col gap-2 items-center justify-center w-1/5 ">
        <h1 className="text-2xl max-xsm:text-[20px] xsm:text-[22px] font-bold text-orange-600">Projects</h1>
        <hr className=" w-full" />
      </div>

      <div className="max-xsm:flex max-xsm:flex-col max-xsm:gap-4">
      {ProjectDetails.map(project =>(
        <div key={project.id} className="bg-[#121214] w-[90%] rounded-3xl m-auto p-6">
        <div className="flex flex-col gap-2 max-xsm:p-2 p-4">
          <h2 className="text-2xl xsm:text-[20px] font-bold max-xsm:text-[18px]">{project.name}</h2>
